...
async def window_glare(req: WindowGlareRequest):
    ...
    try:
        ...
        # ── Edge-bounded region confinement ────────────────────
        # tier2/tier3 alone are ambiguous — a sunlit wall or a bright
        # reflection can be just as bright as real glare without ever
        # truly blowing out. Only tier1 (full blowout, every channel
        # >230) is trusted as unambiguous glare. Build regions separated
        # by strong edges (window frames, wall corners) and only let
        # tier2/tier3 count as glare where they're connected — through
        # open, edge-free space — to an actual tier1 seed. This is what
        # stops correction from reaching a wall: the previous edge
        # barrier only softened the correction near a detected edge
        # after the mask had already been built (and a 14px morphological
        # close can merge a window blob with nearby bright wall pixels
        # before that barrier ever applies) — this instead removes
        # unconnected tier2/tier3 pixels before they're used at all.
        edges = cv2.Canny(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), 40, 120)
        edge_wall = cv2.dilate(edges, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))
        edge_wall = cv2.morphologyEx(edge_wall, cv2.MORPH_CLOSE,
                                      cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)))
        free_space = cv2.bitwise_not(edge_wall)
        n_regions, region_labels = cv2.connectedComponents(free_space, connectivity=4)

        if np.any(tier1_clean > 0):
            if n_regions > 1:
                seed_regions = set(np.unique(region_labels[tier1_clean > 0])) - {0}
                confine = np.isin(region_labels, list(seed_regions)).astype(np.uint8) * 255
                confine = cv2.dilate(confine, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9)))
                tier2_clean = cv2.bitwise_and(tier2_clean, confine)
                tier3_clean = cv2.bitwise_and(tier3_clean, confine)
        else:
            # No unambiguous full blowout anywhere in the photo — tier2/
            # tier3 on their own are too likely to just be bright,
            # correctly-exposed surfaces (sunlit walls, fixtures) to
            # safely correct.
            tier2_clean = np.zeros_like(tier2_clean)
            tier3_clean = np.zeros_like(tier3_clean)
        ...
        # Edge-aware boundary: soften further right at detected edges
        # (window frame into wall) for a natural transition within the
        # already-confined region.
